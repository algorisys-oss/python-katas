"""
Kata 37 -- ASGI Primer
Run: python playground/37_asgi_primer.py
...
def wsgi_app(environ: dict, start_response: callable) -> list[bytes]:
    ...
    Problems with WSGI:
    - Synchronous only -- blocks the thread during I/O
    - No WebSocket support
    - No HTTP/2 server push
    - One request = one thread (expensive)
    """
    status = "200 OK"
...
async def asgi_app(scope: dict, receive: callable, send: callable) -> None:
    ...
    ASGI (PEP-style spec, not a PEP) is asynchronous:
    ...
    - WebSocket support (scope["type"] == "websocket")
    - HTTP/2 support
...
def demo_wsgi_vs_asgi():
    ...
    # Mock receive: returns request body (empty for GET)
    async def mock_receive():
        return {"type": "http.request", "body": b"", "more_body": False}

    # Mock send: captures what the app sends
...
def make_http_scope(
    ...
    The scope dict describes the incoming connection. For HTTP requests:
    - type: always "http"
    - asgi: protocol version info
    - http_version: "1.0", "1.1", or "2"
    - method: HTTP method (GET, POST, etc.)
    - path: URL path
    - query_string: bytes after the ? in the URL
    - headers: list of [name, value] byte pairs (lowercase names!)
...
def make_websocket_scope(path: str = "/ws") -> dict[str, Any]:
    ...
    WebSocket connections have a different type but similar structure.
    """
    return {
        "type": "websocket",
        "asgi": {"version": "3.0"},
...
class AsgiSimulator:
    ...
    async def receive(self) -> dict:
        ...
        For HTTP, the server sends:
        - {"type": "http.request", "body": b"...", "more_body": False}
        For chunked transfers, more_body=True until the last chunk.
        """
        ...
        # After body is sent, receive blocks (simulated by returning disconnect)
        return {"type": "http.disconnect"}

    async def send(self, message: dict) -> None:
        """Capture messages sent by the app.

        For HTTP responses, the app sends two messages:
        1. {"type": "http.response.start", "status": 200, "headers": [...]}
        2. {"type": "http.response.body", "body": b"..."}
        """
...
async def routing_asgi_app(scope: dict, receive: callable, send: callable) -> None:
    ...
    Shows how a framework dispatches requests to different handlers
    based on the scope's path.
    """
    if scope["type"] != "http":
        return
    ...
    if path == "/" and method == "GET":
        response_body = b"Welcome to ASGI!"
        status = 200
        ...
        response_body = json.dumps({"message": "Hello", "protocol": "ASGI"}).encode()
        status = 200
    else:
        response_body = b"Not Found"
        status = 404

    # Send response (always two messages for HTTP)
...
async def app_with_lifespan(scope: dict, receive: callable, send: callable) -> None:
    ...
    The server sends lifespan events with scope["type"] == "lifespan".
    """
    if scope["type"] == "lifespan":
        while True:
            ...
            if message["type"] == "lifespan.startup":
                ...
                try:
                    scope["state"]["db_pool"] = "PostgresPool(connected)"
                    scope["state"]["cache"] = {"warm": True}
...
async def _simulate_lifespan():
    ...
    # Run lifespan (processes both startup and shutdown)
    ...
    # --- HTTP phase (simulate a request while app is "running") ---
    # Reset state to simulate mid-lifecycle
