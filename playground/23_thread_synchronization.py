...
Run: python playground/23_thread_synchronization.py
...
def bounded_buffer_demo() -> tuple[list[str], list[str]]:
    ...
    def producer(name: str, items: list[str]):
        for item in items:
            buffer.put(item)  # Blocks if buffer is full (backpressure)
            with produced_lock:
                produced.append(f"{name}:{item}")
            time.sleep(0.01)
        buffer.put(None)  # Sentinel: "I'm done producing"
...
def deadlock_demo() -> bool:
    ...
    def thread_1():
        ...
        if not lock_b.acquire(timeout=0.05):
            deadlocked.set()  # Could not get lock_b -- deadlock!
            lock_a.release()
            return
        lock_b.release()
        lock_a.release()
    ...
    def thread_2():
        ...
        time.sleep(0.01)  # Give thread_1 time to grab lock_a
        if not lock_a.acquire(timeout=0.05):
            deadlocked.set()  # Could not get lock_a -- deadlock!
            lock_b.release()
            return
        lock_a.release()
        lock_b.release()
...
def no_deadlock_demo() -> list[str]:
    ...
    def worker(name: str):
        # Always acquire lock_a first, then lock_b -- no circular wait
        with lock_a:
            time.sleep(0.01)
            with lock_b:
                with results_lock:
...
if __name__ == "__main__":
    ...
    print()
    print("All 5 sections passed. You've mastered thread synchronization!")
