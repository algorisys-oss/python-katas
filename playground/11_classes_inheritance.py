...
Run: python playground/11_classes_inheritance.py
...
if __name__ == "__main__":
    ...
    # mro() method returns the same as a list
    mro_list = D.mro()
    assert mro_list == list(D.__mro__)

    # Verify C3 properties
    # 1. Children before parents
    assert mro_names.index("D") < mro_names.index("B")
    assert mro_names.index("B") < mro_names.index("A")
    # 2. Left-to-right preserved
    assert mro_names.index("B") < mro_names.index("C")
    ...
    # super() in B2 calls C2 (not A2!) because D2's MRO is D2->B2->C2->A2

    # Demonstrate: same B2 class, different behavior based on instance type
    result_b = B2().method()
    print(f"  B2().method() = {result_b!r}")
    assert result_b == "B -> A"
    # When called on a B2 instance, B2's MRO is B2->A2, so super() calls A2
