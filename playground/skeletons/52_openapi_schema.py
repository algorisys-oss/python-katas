...
Run: python playground/skeletons/52_openapi_schema.py
...
class OpenAPIGenerator:
    ...
    def generate(self, routes: list[RouteInfo]) -> dict[str, Any]:
        ...
        # TODO: Build the complete OpenAPI spec dict
        # Structure:
        # {
        #     "openapi": "3.0.3",
        ...
        #     "paths": paths,
        # }
        # If self.description, add it to info
        # If schemas dict is not empty, add: "components": {"schemas": schemas}
        spec: dict[str, Any] = {}
    ...
    def _build_operation(
        ...
        # TODO: Build the operation dict with:
        # - "summary": route.summary
        # - "operationId": route.handler.__name__
        # - "description": route.description (if not empty)
        # - "tags": route.tags (if not empty)
        # - "deprecated": True (if route.deprecated)
        # - "parameters": extracted params list (if any)
        # - "requestBody": body schema (if handler takes a BaseModel param)
    ...
    def _extract_params(
        ...
        for name, param in sig.parameters.items():
            ...
            # TODO: Body model detection
            # If annotation is a type and subclass of BaseModel:
            #   Generate schema: model_schema = annotation.schema()
            #   Store in schemas dict: schemas[annotation.__name__] = model_schema
            #   Set body_schema to {"$ref": f"#/components/schemas/{name}"}
            #   continue

            # TODO: Path parameter detection
            # If name is in path_param_names:
            #   Create param dict: {"name": name, "in": "path", "required": True,
            #     "schema": {"type": PYTHON_TYPE_MAP.get(annotation, "string")}}
            #   Append to params list; continue

            # TODO: Query parameter detection
            # Build query_schema with type from PYTHON_TYPE_MAP
            # Check if param.default is a Query marker -> merge constraints
            # Determine if required (no default = required)
            # Create param dict: {"name": name, "in": "query", ...}
    ...
    def _build_responses(
        ...
        # TODO: If route.response_model exists and is a BaseModel subclass:
        # Generate its schema, store in schemas dict, and add a $ref to response
        # response["content"] = {"application/json": {"schema": {"$ref": ...}}}

        responses: dict[str, Any] = {status: response}

        # TODO: Add 422 error response for POST/PUT/PATCH methods
