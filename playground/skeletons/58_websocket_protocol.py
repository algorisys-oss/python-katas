...
Run: python playground/skeletons/58_websocket_protocol.py
...
def compute_accept_key(client_key: str) -> str:
    ...
    """
    # TODO: Concatenate client_key with WEBSOCKET_GUID
    # TODO: Compute SHA-1 hash of the combined string (encode to ascii first)
    # TODO: Base64-encode the hash digest and return as string
    # HINT: hashlib.sha1(...).digest() then base64.b64encode(...)
    pass
...
def validate_upgrade_request(headers: dict[str, str]) -> bool:
    ...
    # TODO: Check that all of these are true:
    # - headers["upgrade"] is "websocket" (case insensitive)
    # - headers["connection"] contains "upgrade" (case insensitive)
    # - "sec-websocket-key" exists in headers
    # - headers["sec-websocket-version"] is "13"
    pass
...
class WebSocketFrame:
    ...
    def is_text(self) -> bool:
        # TODO: Return True if opcode is TEXT (0x1)
        pass

    @property
    def is_binary(self) -> bool:
        # TODO: Return True if opcode is BINARY (0x2)
        pass

    @property
    def is_close(self) -> bool:
        # TODO: Return True if opcode is CLOSE (0x8)
        pass

    @property
    def is_ping(self) -> bool:
        # TODO: Return True if opcode is PING (0x9)
        pass

    @property
    def is_pong(self) -> bool:
        # TODO: Return True if opcode is PONG (0xA)
        pass
...
def apply_mask(data: bytes, mask_key: bytes) -> bytes:
    ...
    """
    # TODO: XOR each byte of data with the corresponding byte of mask_key
    # HINT: bytes(b ^ mask_key[i % 4] for i, b in enumerate(data))
    pass
...
def decode_frame(data: bytes) -> tuple[WebSocketFrame, int]:
    ...
    # TODO: Parse byte 0: FIN bit (bit 7) and opcode (bits 0-3)
    # HINT: fin = bool(data[0] & 0x80), opcode = data[0] & 0x0F
    byte0 = data[0]
    fin = False    # TODO: Extract FIN bit (bit 7)
    opcode = 0     # TODO: Extract opcode (bits 0-3)

    # TODO: Parse byte 1: MASK bit (bit 7) and payload length (bits 0-6)
    byte1 = data[1]
    masked = False        # TODO: Extract MASK bit (bit 7)
    payload_length = 0    # TODO: Extract payload length (bits 0-6)

    offset = 2

    # TODO: Handle extended payload length
    # If payload_length == 126: read next 2 bytes as 16-bit unsigned big-endian
    #   HINT: struct.unpack("!H", data[offset:offset+2])[0]
    # If payload_length == 127: read next 8 bytes as 64-bit unsigned big-endian
    #   HINT: struct.unpack("!Q", data[offset:offset+8])[0]

    # TODO: If masked, read the 4-byte masking key from data[offset:offset+4]
    mask_key = None

    # TODO: Read the payload bytes from data[offset:offset+payload_length]
    payload = b""

    # TODO: If masked, unmask the payload using apply_mask(payload, mask_key)
...
def encode_frame(
    ...
    # TODO: Build byte 0: set FIN bit (0x80) if fin is True, OR with opcode
    byte0 = opcode
    # TODO: if fin: byte0 |= 0x80
    frame.append(byte0)

    # TODO: Build byte 1: set MASK bit (0x80) if masked, encode payload length
    length = len(payload)
    mask_bit = 0x00  # TODO: Set to 0x80 if masked

    # TODO: If length < 126: append mask_bit | length
    # If length < 65536: append mask_bit | 126, then pack length as 16-bit
    # Else: append mask_bit | 127, then pack length as 64-bit
    # HINT: struct.pack("!H", length) for 16-bit, struct.pack("!Q", length) for 64-bit
    frame.append(mask_bit | length)

    # TODO: If masked, append mask_key bytes and apply masking to payload
    # HINT: frame.extend(mask_key) then payload = apply_mask(payload, mask_key)
