...
Run: python playground/51_type_driven_validation.py
...
# It works as both a default-value marker and a descriptor that validates
# on attribute set.
    ...
    def validate(self, value: Any) -> list[str]:
        ...
        if isinstance(value, str):
            if self.min_length is not None and len(value) < self.min_length:
                errors.append(
                    f"{self._name}: length {len(value)} < minimum {self.min_length}"
                )
            if self.max_length is not None and len(value) > self.max_length:
                errors.append(
                    f"{self._name}: length {len(value)} > maximum {self.max_length}"
                )
            if self.pattern and not self.pattern.match(value):
                errors.append(
                    f"{self._name}: value does not match pattern {self.pattern_str!r}"
                )
        ...
        if isinstance(value, (int, float)):
            ...
            if self.le is not None and value > self.le:
                errors.append(f"{self._name}: {value} > maximum {self.le}")
            if self.gt is not None and value <= self.gt:
                errors.append(f"{self._name}: {value} must be > {self.gt}")
            if self.lt is not None and value >= self.lt:
                errors.append(f"{self._name}: {value} must be < {self.lt}")

        # Collection size checks
        if isinstance(value, (list, tuple, set)):
            if self.min_items is not None and len(value) < self.min_items:
                errors.append(
                    f"{self._name}: {len(value)} items < minimum {self.min_items}"
                )
            if self.max_items is not None and len(value) > self.max_items:
                errors.append(
                    f"{self._name}: {len(value)} items > maximum {self.max_items}"
                )
...
# ===========================================================================
# A model base class that validates all fields on construction.
...
class ValidatedModel:
    ...
    def __init__(self, **kwargs: Any):
        ...
        for field_name, field_type in hints.items():
            ...
            elif field_desc and field_desc.default is not _MISSING:
                value = field_desc.default
            elif hasattr(type(self), field_name) and not isinstance(
                getattr(type(self), field_name, None), Field
            ):
            ...
            if not isinstance(value, field_type) and value is not None:
                try:
                    ...
                    errors.append({
                        "field": field_name,
                        "message": f"expected {field_type.__name__}, got {type(value).__name__}",
                        "type": "type_error",
                    })
                    continue
...
class Query:
    ...
    Usage in handler signatures:
        def search(q: str = Query(min_length=1), limit: int = Query(ge=1, le=100)):
            ...
    """
...
class ValidatedInjector:
    ...
    def __init__(self, path_template: str, handler: Callable):
        ...
        self.hints = get_type_hints(handler)
