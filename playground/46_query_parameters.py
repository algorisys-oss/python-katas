...
Run: python playground/46_query_parameters.py
...
def parse_query_string(query: str) -> dict[str, list[str]]:
    ...
    Returns:
        Dict mapping parameter names to lists of string values.
    """
...
def coerce_query_value(
    ...
    if is_list:
        # Coerce each element
        return [_coerce_single(v, item_type, param_name) for v in raw_values]

    # Scalar: use the last value (standard behavior)
    if not raw_values:
        raise QueryParamError(param_name, "no value provided")
    return _coerce_single(raw_values[-1], target_type, param_name)
...
def _coerce_single(value: str, target_type: type, param_name: str) -> Any:
    ...
    if target_type is bool:
        ...
        raise QueryParamError(
            param_name, f"cannot convert '{value}' to bool"
        )
    ...
    if target_type is int:
        try:
            ...
            raise QueryParamError(
                param_name, f"cannot convert '{value}' to int"
            )
    ...
    if target_type is float:
        try:
            ...
            raise QueryParamError(
                param_name, f"cannot convert '{value}' to float"
            )
...
def extract_query_params(
    ...
    *,
    ...
    Args:
        func: The route handler function to inspect
        query_string: Raw query string (without '?')
        skip_params: Parameter names to skip (e.g., 'request', 'self')
    ...
    for param_name, param in sig.parameters.items():
        ...
        if param_name in raw_params:
            try:
                result[param_name] = coerce_query_value(
                    raw_params[param_name], annotation, param_name
                )
            ...
            errors.append(
                f"Query parameter '{param_name}': required but not provided"
            )
...
# ===========================================================================
# Show how this integrates with route handlers in the Ignite framework.
...
class Request:
    ...
    def __init__(self, method: str, path: str, query_string: str = ""):
        self.method = method
        # Split path and query string if combined
...
def demo_signature_extraction():
    ...
    # Define a handler with typed parameters
    def search_items(
        request: Request,
        q: str,
        page: int = 1,
    ...
    assert params["sort"] == "relevance"  # default
    assert params["active"] is None  # optional, not provided
